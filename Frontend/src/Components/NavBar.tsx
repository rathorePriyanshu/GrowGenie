...
const NavBar = () => {
  ...
  const { setStream } = useCareerStore();
  ...
  const navItems = navlinks.map((link, index) => (
    <div key={link.name} className="relative flex items-center">
      <Link
        ...
        className="px-3 py-1 text-white font-semibold bg-transparent rounded-md shadow-md hover:shadow-xl transform hover:-translate-y-1 transition-all duration-200"
      >
        {link.name}
      </Link>
      ...
      {index !== navlinks.length - 1 && (
        <span className="absolute right-0 top-1/2 transform -translate-y-1/2h-6 border-gray-600"></span>
      )}
    </div>
  ));
